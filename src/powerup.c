...
void SpawnPowerUp(PowerUp *powerUps)
{
    for (int i = 0; i < MAX_POWERUPS; i++)
    {
        if (!powerUps[i].active)
        {
            ...
            powerUps[i].position = (Vector2){ rand() % (800 - 40) + 20, rand() % (450 - 40) + 20 };
...
void UpdatePowerUps(PowerUp *powerUps, Player *player)
{
    for (int i = 0; i < MAX_POWERUPS; i++)
    {
        if (powerUps[i].active && CheckCollisionRecs((Rectangle){player->position.x, player->position.y, player->size.x, player->size.y}, (Rectangle){powerUps[i].position.x, powerUps[i].position.y, powerUps[i].size.x, powerUps[i].size.y}))
        {
            powerUps[i].active = false;
