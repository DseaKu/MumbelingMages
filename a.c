 ...
 *memory (VRAM)
 *
 *   Example originally created with raylib 1.4, last time updated with
 *raylib 1.4
 *
...
int main(void) {
  ...
  ImageDraw(&parrots, cat,
            (Rectangle){0, 0, (float)cat.width, (float)cat.height},
            (Rectangle){30, 40, cat.width * 1.5f, cat.height * 1.5f}, WHITE);
  ImageCrop(&parrots,
            (Rectangle){0, 50, (float)parrots.width,
                        (float)parrots.height - 100}); // Crop resulting image
  ...
  ImageDrawTextEx(&parrots, font, "PARROTS & CAT", (Vector2){300, 230},
  ...
  Texture2D texture = LoadTextureFromImage(
      parrots); // Image converted to texture, uploaded to GPU memory (VRAM)
  UnloadImage(parrots); // Once image has been converted to texture and uploaded
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    DrawText("We are drawing only one texture from various images composed!",
             240, 350, 10, DARKGRAY);
    DrawText("Source images have been cropped, scaled, flipped and copied one "
             "over the other.",
             190, 370, 10, DARKGRAY);
